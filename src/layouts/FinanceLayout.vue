...
<template>
  <div class="flex h-screen overflow-hidden">
    ...
    <aside
      ...
      style="background: linear-gradient(180deg, #1E3A5F 0%, #152C4A 100%)"
    >
      <!-- Logo -->
      ...
      <nav class="flex-1 space-y-1 px-3 py-2">
        ...
        >
          <span
            class="absolute left-0 h-6 w-[3px] rounded-r-full bg-sky-400 opacity-0 transition-opacity duration-200"
            :class="{ '!opacity-100': $route.path === item.to || (item.to !== '/finance' && $route.path.startsWith(item.to)) || (item.to === '/finance' && $route.path === '/finance') }"
          />
