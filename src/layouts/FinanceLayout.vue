...
function isActive(to: string): boolean {
  if (to === '/finance') return route.path === '/finance'
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <header class="flex h-16 items-center justify-between border-b border-border bg-white/95 px-6 backdrop-blur-sm lg:px-8">
        ...
        <div class="flex items-center gap-3">
          ...
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full bg-finance text-sm font-semibold text-white ring-2 ring-border"
          >
            {{ userInitials }}
          </div>
        </div>
